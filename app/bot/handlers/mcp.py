...
async def mcp_command(
    ...
    *,
    ...
    await update.message.reply_html(
        ...
        "<code>/mcp_token new macbook</code>\n"
        "The last word is just a label so you can recognise and revoke this "
        "token later — name it after the device or app you'll use it from "
        "(e.g. <code>macbook</code>, <code>work-cursor</code>). The token is "
        "shown once — copy it.\n\n"
